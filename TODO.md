# Dotfiles TODO

- [x] try alternatives to tab completion
- [x] rename log files: "<yyyymmdd>_<hhmmss>_<script name>_<id>.log"
- [x] 03-1password-setup.sh should check for pub keys and use log files too
- [ ] del/end keys not working in zsh
- [ ] sudo alias resolution
- [ ] sudo zsh completions
- [ ] sudo neovim editing
- [ ] fresh-editor error on ubuntu, install via `sudo snap install fresh-editor --classic`
- [ ] fedora: install `fresh` via terra - [https://github.com/terrapkg/packages]
- [ ] debian: install chezmoi via snap
- [ ] support cachyos
- [ ] automate macos configuration via 'defaults'
- [ ] test usage of ansible over bdb
